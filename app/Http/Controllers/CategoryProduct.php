...
public function all_sanpham(Request $request ){
...
return view('pages.category.show_category')->with('category',$cate_product)->with('brand',$brand_product)->with('category_by_id',$category_by_id)->with('slider',$slider)->with('category_post',$category_post)->with('min_price',$min_price)->with('max_price',$max_price)->with('max_price_range',$max_price_range)->with('min_price_range',$min_price_range);
}
...
public function product_tabs(Request $request){
    ...
    if($product_count>0){
        ...
        foreach($product as $key => $val) {
            ...
            <a href="'.url('chi-tiet/'.$val->product_slug).'">
            <div class="col-sm-4">
            ...
            <div class="single-product">
                            <div class="product-img">
                                <a href="'.url('chi-tiet/'.$val->product_slug).'">
                                    ...
                                    <img class="hover-img" src="'.url('public/uploads/product/'.$val->product_image).'" alt="'.$val->product_name.'">
                                </a>
								<div class="button-head">
									...
									<div class="product-action-2">
										<a title="Thêm giỏ hàng" href="#"  id="'.$val->product_id.'" onclick="Addtocart(this.id);">Thêm giỏ hàng</a>
									</div>
								</div>
                            </div>
                            <div class="product-content">
                                <h3><a> '.$val->product_name.'</a></h3>
                                <div class="product-price">
                                    <span>'.number_format($val->product_price,0,',','.').' VNĐ</span>
                                </div>
                            </div>
                        </div>
						<!-- End Single Product -->
