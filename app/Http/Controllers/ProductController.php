...
class ProductController extends Controller
{
   ...
   public function file_browser(Request $request){
    ...
    public function quickview(Request $request){
        ...
        // $output['product_cost'] = number_format($product->product_cost,0,',','.').'VNĐ';
...
public function import_csv(Request $request){
    ...
    public function show_product_home(Request $request ,$slug_category_product){
...
public function index(Request $request){
    ...
    $cate_pro_tabs = CategoryProductModel::where('category_parent',0)->orderBy('category_id','DESC')->get();
    // Xác định giá thấp nhất và cao nhất của sản phẩm trong toàn bộ danh sách
    $min_price = Product::min('product_price');
    $max_price = Product::max('product_price');
    ...
    $max_price_range = $max_price + 10000000;
      ...
      if(isset($_GET['sort_by'])){
        ...
        if($sort_by=='giam_dan'){
            ...
            $category_by_id = Product::with('category')->orderBy('product_price','DESC')->paginate(6)->appends(request()->query());

        }elseif($sort_by=='tang_dan'){

          $category_by_id = Product::with('category')->orderBy('product_price','ASC')->paginate(6)->appends(request()->query());

      }elseif($sort_by=='kytu_za'){

       $category_by_id = Product::with('category')->orderBy('product_name','DESC')->paginate(6)->appends(request()->query());


   }elseif($sort_by=='kytu_az'){

    $category_by_id = Product::with('category')->orderBy('product_name','ASC')->paginate(6)->appends(request()->query());
}
...
$category_by_id = Product::with('category')->orderBy('product_id','DESC')->paginate(6);
}
...
return view('pages.sanpham.sanpham')->with('category',$cate_product)->with('brand',$brand_product)->with('category_by_id',$category_by_id)->with('meta_desc',$meta_desc)->with('meta_keywords',$meta_keywords)->with('meta_title',$meta_title)->with('url_canonical',$url_canonical)->with('slider',$slider)->with('category_post',$category_post)->with('min_price',$min_price)->with('max_price',$max_price)->with('max_price_range',$max_price_range)->with('min_price_range',$min_price_range);


    // return view('pages.sanpham.sanpham')
